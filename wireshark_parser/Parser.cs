...
internal class Parser : IParser
{
    ...
    /// <summary>
    /// Uses SharpPcap to parse the pcap file with an offline pcap reader device. Using event to parse the packets.
    ...
    internal void Parse(string pcapFile, string protocol)
    {
        ...
        using (Operation.Time("Parsing packets and storing to DB from {pcapFile}", pcapFile))
        {
            ...
            // Since we need certain types of protocols, I am prefiltering the data with a BPF filter,
            // which hopefully leaves only packets of the target protocol. Filtering is based on ports that
            // the protocols use. For example, filter "icmp" should leave only packets with ICMP protocols.
    ...
    /// <summary>
    /// Gets the time, src ip, src port, dst ip and dst port, protocol name and http method
    ...
    private void device_OnPacketArrival(object sender, PacketCapture e)
    {
        ...
        var time = e.Header.Timeval.Date.ToUniversalTime();
        ...
        if (tcpPacket != null)
        {
            ...
            var tempPacket = new Entities.Packet()
            {
                ...
                IPSource = srcIp.ToString(),
                IPDestination = dstIp.ToString(),
                // This is wrong, but must leave for now.
            ...
            {
                Console.WriteLine(System.Text.Encoding.UTF8.GetString(packet.PayloadData));
            }

            Console.WriteLine("----------------\n");

            _log.LogInformation("{protocol} // {hour}:{minute}:{second},{millisecond} Len={len} " +
                "{srcIp}:{srcPort} -> {dstIp}:{dstPort} Packet index: {packetIndex}", Protocol,
                time.Hour, time.Minute, time.Second, time.Millisecond, len,
                srcIp, srcPort, dstIp, dstPort, _packetIndex);
        }
    }
}
